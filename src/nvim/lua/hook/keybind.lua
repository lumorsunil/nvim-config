...
function keybind_table.general()
  ...
  nmap({
    ...
    ["<F20>"] = mkbind(function()
      vim.diagnostic.jump({ count = -1 })
    end, "diagnostic.goto_prev"),
    ["gx"] = "<Cmd>URLOpenUnderCursor<CR>",
    ["<leader>vc"] = mkbind(function()
      vim.cmd([[let @/ = ""]])
      vim.cmd([[silent! NvimTreeClose]])
      vim.cmd([[silent! OutlineClose]])
    end, "clear search highlights, temp windows, etc"),
    ...
    ["<leader>sl"] = mkbind(
      "<Cmd>silent! source session.vim | silent! source session.lua<CR>",
...
local function session_switch(n)
  return function()
    local sessions = require("init_all.sessions")
    local state = sessions.state()
    local current_index = 1

    if state.current then
      current_index = state.current.index
    end

    vim.notify("current_index = " .. current_index .. " #sessions = " .. #state.sessions .. " n = " .. n)
    ...
    if #state.sessions == 1 then
      sessions.switch_to_server(state.sessions[1])
      return
    end
    ...
    if new_index < 1 then
      new_index = new_index + #state.sessions
    end
    new_index = new_index % (#state.sessions + 1)
    if new_index == 0 then
      new_index = 1
    end

    local new_session = state.sessions[new_index]

    sessions.switch_to_server(new_session)
  end
end

-- @sessions.keybind
function keybind_table.sessions()
  map({ "n", "i", "x", "c", "o", "t" }, {
    ["<C-Tab>"] = mkbind(session_switch(1), "Switch session right"),
    ["<C-S-Tab>"] = mkbind(session_switch(-1), "Switch session left"),
  }, {})
end
...
function keybind_table.telescope()
  ...
  nmap({
    ...
    ["gd"] = mkbind(
      alternative(with(builtin.lsp_definitions, themes.get_ivy()), vim.lsp.buf.definition),
      "Definitions"
    ),
    ["gr"] = mkbind(alternative(with(builtin.lsp_references, themes.get_ivy()), vim.lsp.buf.references), "References"),
    ["gi"] = mkbind(
      alternative(with(builtin.lsp_implementations, themes.get_ivy()), vim.lsp.buf.implementation),
      "Implementations"
    ),
    ["<space>D"] = mkbind(
      alternative(with(builtin.lsp_type_definitions, themes.get_ivy()), vim.lsp.buf.type_definition),
      "Type definitions"
    ),
    ["<leader>fr"] = mkbind("<Cmd>Telescope smart_open<CR>", "Smart open"),
    ["<leader>fp"] = mkbind(remote_plugins, "Remote plugins neocraft"),
    -- ["<leader>fs"] = mkbind(
    --   alternative(builtin.lsp_workspace_symbols, vim.lsp.buf.workspace_symbol),
    --   "Workspace symbols"
    -- ),
    ["<leader>fs"] = mkbind("<Cmd>Telescope sessions<CR>", "Sessions"),
    ["<leader>fd"] = mkbind(alternative(builtin.lsp_document_symbols, vim.lsp.buf.document_symbol), "Document symbols"),
...
function keybind_table.cmp()
  ...
  return cmp.mapping.preset.insert(
    vim.tbl_deep_extend("force", cmp_standard_bindings(cmp), cmp_insert_only_bindings(cmp))
  )
end
...
function keybind_table.treesitter()
  ...
  local util_maps = {
    ...
    ["<leader>tff"] = mkbind(
      ...
      "treesitter open find files for node"
    ),
    ["<leader>tfg"] = mkbind(
      with_node(function(node)
        local text = getTrimmedText(vim.treesitter.get_node_text(node, 0))
...
function keybind_table.treesitter_textobjects()
  ...
  function mkselectbind(textobject, desc)
    return mkbind(function()
  ...
  map({ "x", "o" }, {
    ...
    ["af"] = mkselectbind("@function.outer", "Around function"),
    ["if"] = mkselectbind("@function.inner", "Inner function"),
    ["ac"] = mkselectbind("@class.outer", "Around class"),
    ["ic"] = mkselectbind("@class.inner", "Inner class"),
    -- You can also use captures from other query groups like `locals.scm`
    -- ["as"] = { query = "@scope", query_group = "locals", desc = "Select language scope" },
    ["aa"] = mkselectbind("@parameter.outer", "Around parameter"),
    ["ia"] = mkselectbind("@parameter.inner", "Inner parameter"),
    ["a#"] = mkselectbind("@comment.outer", "Around comment"),
    ["i#"] = mkselectbind("@comment.inner", "Inner comment"),
  })
...
function keybind_table.perfanno()
  return {
    ...
    },
    {
      ...
      desc = "Hottest callers selection",
      noremap = true,
      silent = true,
    },
  }
